    ...
    stages {
		...
		stage('Stop PTJournal AppPool') {
            steps {
				...
				powershell('if ($(C:\Windows\system32\inetsrv\appcmd.exe list site PTJournal /text:state) -eq "Started") {%windir%\\system32\\inetsrv\\appcmd stop apppool /apppool.name:"PTJournal"}')
            }
        }
