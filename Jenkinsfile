    ...
    stages {
		...
		stage('Stop PTJournal AppPool') {
            steps {
				...
				powershell('if ($(C:\\Windows\\system32\\inetsrv\\appcmd.exe list site PTJournal /text:state) -eq "Started") {C:\\Windows\\system32\\inetsrv\\appcmd.exe stop apppool /apppool.name:"PTJournal"}')
            }
        }
        ...
        stage('Delete IIS Journal Directory') {
            steps {
                retry(5) {
                    bat('if exist "C:\\Bin\\IIS\\PTJournal" RMDIR /Q/S "C:\\Bin\\IIS\\PTJournal"')
                }
